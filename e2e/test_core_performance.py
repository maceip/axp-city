...
Opt in with --performance: the selected browser, with a 100ms p95 timing budget.
...
def test_populated_core_frame_budget_and_resource_plateau(core_server, new_context, browser, browser_name, browser_type_launch_args):
    started = time.monotonic()
    backend = dict(backend=f"local-{browser_name}", browser=browser.version, engine=browser_name,
                   headed=not browser_type_launch_args.get("headless", True))
    ...
    context = new_context(viewport=dict(width=1600, height=1000), device_scale_factor=1,
    ...
    report = dict(profile="hardware target" if HARDWARE else "browser timing budget", backend=backend,
