...
export function stampImage(
  ...
  img.setAlpha(stamp.alpha ?? (stamp.dimmed ? 0.62 : 1));
  if (stamp.pixelated) {
...
export function stampAnim(
  ...
  const cellW =
    stamp.anim === "humanWalk" ? 24 : stamp.sheet.width / stamp.sheet.cols;
  if (!active)
    return scene.add
      .image(stamp.sx, stamp.sy, stamp.anim, 0)
      .setOrigin(0.5, 1)
      .setScale(stamp.targetW / cellW)
      .setDepth(stamp.depth);
  ...
  const frameCount = stamp.anim === "humanWalk" ? 4 : stamp.sheet.frames;
  const loopDur =
    frameCount / (stamp.anim === "humanWalk" ? 5 : stamp.sheet.fps);
  const phase = ((stamp.phase % loopDur) + loopDur) % loopDur;
  const startFrame = Math.floor((phase / loopDur) * frameCount) % frameCount;
