...
export class HudScene extends Phaser.Scene {
  ...
  private cityTitle!: Phaser.GameObjects.BitmapText;
  ...
  update(): void {
    ...
    if (stamp !== this.constructionStamp) this.setSelection(this.selected, false);
  }
  ...
  private layout(): void {
    ...
    // Phones stack plate → search → status so the native field is not under the survey plate.
  ...
  setSelection(place: LotPlacement | undefined, announce = true): void {
    ...
    const lines: Array<[string, string, string?, boolean?]> = [
      ...
      [`STARS ${lot.stars.toLocaleString()}    ISSUES ${lot.openIssues}    OPEN PRS ${lot.openPrs}`, INK, "12px", true],
      [site.stage !== "complete" ? `UNDER CONSTRUCTION · ${constructionLabel(site).toUpperCase()} · ${Math.round(site.progress * 100)}%` : yardLabel(lot.yard).toUpperCase(), GOLD, "11px", true],
      [crewLabel(lot), INK, "11px"],
      [lot.crewBasis, MUTED, "10px", true],
   ...
   * covered controls are withdrawn rather than left showing through the plaque.
   */
  ...
  /** Toast wraps inside the viewport; on a phone it hangs under the MASS stack. */
  private placeToast(message?: string): void {
    ...
    if (message !== undefined) {
      this.toastText.setMaxWidth(width - 36);
  ...
  locate(name: string): { x: number; y: number; width: number; height: number } | null {
    const object =
      ...
      (this.children.getByName(name) as Phaser.GameObjects.Container | Phaser.GameObjects.Image | null) ??
      (this.census.getByName(name) as Phaser.GameObjects.GameObject | null);
