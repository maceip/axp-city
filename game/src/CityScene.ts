...
export class CityScene extends Phaser.Scene {
  ...
  private exposeDiagnostics(): void {
    (window as unknown as { __AXP: unknown }).__AXP = {
      ...
      diagnostics: () => ({
        ...
        assetBytesRequested: this.assets.bytesRequested,
        sheetsRequested: this.assets.sheetsRequested,
        ...
        drawnLabels: this.civics
          ...
          .map((object) => {
            const stamp = object as Phaser.GameObjects.Container;
            return {
              id: stamp.getData("mapLabel") as string,
              text: stamp.getData("mapLabelText") as string,
              x: stamp.x,
              y: stamp.y,
              depth: stamp.depth,
  ...
  select(repo: string | undefined, frame: boolean): boolean {
    ...
    if (this.hudReady && this.hud.censusIsOpen && frame) this.hud.toggleCensus(false);
