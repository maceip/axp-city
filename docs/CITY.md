...
- `GET /api/city` (= `/api/city/export.json`): `{ version: 1, schema: { snapshot, layout }, revision, serverTime, mode, plan, freshness }`. `plan.placements` contains each `CityLot`, its canonical `col`/`row` and world coordinates, district, and construction timestamp; the plan also carries features, bounds, street rows, and vacancies. `freshness` reports the last successful GitHub refresh, last failure, stale threshold, failing repositories, and the data source (`github-app`, `github-token`, `github-anonymous`, `fixture`).
