...
Automatic CI runs unit/service tests, typecheck, build and backend operations checks. The browser matrix is manual during core development: its last Chromium job took 9.5 minutes, including 8 minutes 36 seconds for 15 functional cases. The retained suite uses standard pytest-playwright fixtures and pinned browsers, following [Streamlit](https://github.com/streamlit/streamlit/blob/a807ca5f8274de0eb708729cb40266f6321b82cc/.github/workflows/playwright.yml#L27-L29). Run it before milestones or relevant renderer/input/save changes; a green automatic check does not establish rendering or browser interactions. Timing benchmarks remain explicit local measurements. [Commands and local artifacts](../e2e/README.md).
